...
if (\Bitrix\Main\Loader::includeModule('sale')) {
	if ((!empty($_REQUEST["city"]) && is_string($_REQUEST["city"])) ||
		...
		if (sizeof($arResult) == 0) {
			// ��� ������� ������������ ��������, ����� ����� ����������� ��� ��������
			if ($region == GetMessage("GORILLAS_ADMIN_SUGGESTIONS_MOSKVA"))
				$filter["~REGION_NAME"] = GetMessage("GORILLAS_ADMIN_SUGGESTIONS_MOSKOVSKAA");
			if ($region == GetMessage("GORILLAS_ADMIN_SUGGESTIONS_SANKT_PETERBURG"))
				$filter["~REGION_NAME"] = GetMessage("GORILLAS_ADMIN_SUGGESTIONS_LENINGRADSKAA");
			if ($region == GetMessage("GORILLAS_ADMIN_SUGGESTIONS_SEVASTOPOLQ"))
				$filter["~REGION_NAME"] = GetMessage("GORILLAS_ADMIN_SUGGESTIONS_KRYM");
			if ($region == GetMessage("GORILLAS_ADMIN_SUGGESTIONS_BAYKONUR")) {
				unset($filter["~REGION_NAME"]);
				$filter["~COUNTRY_NAME"] = GetMessage("GORILLAS_ADMIN_SUGGESTIONS_KAZAHSTAN");
			}
