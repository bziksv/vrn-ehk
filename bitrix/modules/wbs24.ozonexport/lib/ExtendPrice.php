...
class ExtendPrice extends Price
{
    public function getPrice($minPrice, $fullPrice)
    {
        ...
        $plusPercent = intval($this->param['plusPercent'] ?? 0);
        $plusAdditionalSum = intval($this->param['plusAdditionalSum'] ?? 0);
    ...
    {
        $oldPricePlusPercent = intval($this->param['oldPricePlusPercent'] ?? 0);
        $oldPrice10kPlusPercent = intval($this->param['oldPrice10kPlusPercent'] ?? 0);
    ...
    {
        $premiumPriceMinusPercent = intval($this->param['premiumPriceMinusPercent'] ?? 0);
    ...
    {
        $newMinPriceMinusPercent = intval($this->param['newMinPriceMinusPercent'] ?? 0);
