...
$MAX_EXECUTION_TIME = (isset($MAX_EXECUTION_TIME) && !$ozon->isDemoMode() ? (int)$MAX_EXECUTION_TIME : 0);
...
if (empty($arRunErrors))
{
	...
	do
	{
		...
		while ($row = $iterator->Fetch())
		{
            if ($ozon->isLimitOfElementsExpired()) break;
				...
				if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
                        ...
                        if ($allowShow) {
                            ...
                            $allowShow = $ozonFilter->verifyElementShowing($offer, $row);
                        }
